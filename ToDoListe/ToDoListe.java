package MySpace.ToDoListe;

import java.io.BufferedReader;
import java.io.InputStreamReader;
import MySpace.Farben;

public class ToDoListe {
    /*
     * Methoden
     */
    public static void ToDoListeGetInput(){
        //Modusabfrage
        System.out.println(Farben.color_white_underlined + "Willkommen bei der ToDo-Liste!" + Farben.reset);
        System.out.println("Bitte gib " + Farben.color_green + "[1]" + Farben.reset + " für eine neue ToDo ein.");
        System.out.println("Bitte gib " + Farben.color_green + "[2]" + Farben.reset + " um eingetragene ToDo's anzuzeigen.");
        System.out.println("Bitte gib " + Farben.color_green + "[3]" + Farben.reset + " für die Einstellungen.");
        //Input der Modusabfrage auswerten
        switch (betterScanner()) {
            //Neues Event
            case 1:
                ToDoListe_NeueToDo.neueToDo();
                break;
            //Events abfragen
            case 2:
                ToDoListe_FragenNachFiltern.toDoListe_FragenNachFiltern();
                break;
            //Einstellungen
            case 3:
                
                break;
            default:
                System.out.println(Farben.color_red + "Diesen Modus gibt es leider nicht. Bitte wiederhole deine Eingabe." + Farben.reset);
                ToDoListeGetInput();
                break;
        }
    }


    /*
     * Spezieller Verbesserter Scanner.
     */
    public static int betterScanner()
    {
        BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
        
        int eingabe = 0;
        while(true)
        {
        try
        {
            eingabe = Integer.parseInt(reader.readLine());
            break;
        }
        catch(Exception e)
        {
            System.out.println("Ungültige Eingabe!\n");
        }
        }
        return eingabe;
    }
}